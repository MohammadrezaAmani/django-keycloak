...
class ServicesRealmRefreshWellKnownOIDCTestCase(MockTestCaseMixin, TestCase):
    def setUp(self):
        self.realm = RealmFactory(name="test-realm", _well_known_oidc="empty")

        keycloak_oidc_mock = mock.MagicMock()
        keycloak_oidc_mock.well_known.contents = {"key": "value"}
        self.setup_mock(
            "keycloak.realm.KeycloakRealm.open_id_connect",
    ...
    def test_refresh_well_known_oidc(self):
        ...
        django_keycloak.services.realm.refresh_well_known_oidc(realm=self.realm)
