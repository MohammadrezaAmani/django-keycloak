...
class ServicesKeycloakOpenIDProfileUpdateOrCreateTestCase(MockTestCaseMixin, TestCase):
    def setUp(self):
        self.client = ClientFactory(
            realm___certs="{}", realm___well_known_oidc='{"issuer": "https://issuer"}'
        )
        self.client.openid_api_client = mock.MagicMock(spec_set=KeycloakOpenidConnect)
    ...
    def test_create(self):
        django_keycloak.services.oidc_profile.update_or_create_from_code(
            client=self.client, code="some-code", redirect_uri="https://redirect"
        )
        self.client.openid_api_client.authorization_code.assert_called_once_with(
        ...
        self.assertEqual(
            profile.expires_before,
            datetime(year=2018, month=3, day=1, hour=0, minute=10, second=0),
        )
        self.assertEqual(
            profile.refresh_expires_before,
            datetime(year=2018, month=3, day=1, hour=1, minute=0, second=0),
        )
    ...
    def test_update(self):
        ...
        user = UserModel.objects.create(
            username="some-sub", email="", first_name="", last_name=""
        )
