...
class ServicesKeycloakOpenIDProfileGetActiveAccessTokenTestCase(
    ...
    def setUp(self):
        ...
        self.oidc_profile.realm.client.openid_api_client.refresh_token.return_value = {
    ...
    def test_not_expired(self):
        ...
        """
        access_token = django_keycloak.services.oidc_profile.get_active_access_token(
            oidc_profile=self.oidc_profile
        )

        self.assertEqual(access_token, "access-token")
        self.assertFalse(
            self.oidc_profile.realm.client.openid_api_client.refresh_token.called
        )
    ...
    def test_expired(self):
        ...
        self.oidc_profile.realm.client.openid_api_client.refresh_token.assert_called_once_with(
            refresh_token="refresh-token"
        )
        ...
        self.assertEqual(
            self.oidc_profile.expires_before, datetime(2018, 3, 5, 1, 11, 0)
        )
        self.assertEqual(self.oidc_profile.refresh_token, "new-refresh-token")
        self.assertEqual(
            self.oidc_profile.refresh_expires_before, datetime(2018, 3, 5, 2, 1, 0)
        )
