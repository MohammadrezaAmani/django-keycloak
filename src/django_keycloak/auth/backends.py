...
class KeycloakAuthorizationBase(object):
    ...
    def get_keycloak_permissions(self, user_obj):
        ...
        rpt_decoded = django_keycloak.services.oidc_profile.get_entitlement(
                ...
                if "scopes" in p:
                    for scope in p["scopes"]:
                        if "." in p["resource_set_name"]:
                            ...
                            permissions.append(
                                "{app}.{scope}_{model}".format(
                                    app=app, scope=scope, model=model
                                )
                            )
                        else:
                            permissions.append(
                                "{scope}_{resource}".format(
                                    scope=scope, resource=p["resource_set_name"]
                                )
                            )
                else:
...
class KeycloakPasswordCredentialsBackend(KeycloakAuthorizationBase):
    def authenticate(self, request, username, password):
        ...
        try:
            keycloak_openid_profile = django_keycloak.services.oidc_profile.update_or_create_from_password_credentials(
                client=request.realm.client, username=username, password=password
            )
        except KeycloakClientError:
            logger.debug(
                "KeycloakPasswordCredentialsBackend: failed to " "authenticate."
            )
        else:
            return keycloak_openid_profile.user
